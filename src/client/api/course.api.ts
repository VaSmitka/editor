...
export const getCourses = (): Promise<Course[]> => httpApi.get('courses').then(({ data }) => data.data);

export const getCourse = (id: string): Promise<void> => httpApi.get(`courses/${id}`).then((data) => data.data);
...
export const updateCourse = (courseId: string, courseData: any): Promise<any> =>
  httpApi.patch(`courses/${courseId}`, courseData).then((data) => data.data);
